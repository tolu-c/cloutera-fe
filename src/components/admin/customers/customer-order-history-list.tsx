...
export const CustomerOrderHistoryList = ({
  ...
  function selectTable(table: TableType) {
    setTable(table);
  }
    ...
    <div className="flex w-full flex-col items-start gap-4">
      <div className="bg-grey-text-50 flex items-center rounded-lg px-1 py-0.5">
        <button
          type="button"
          className={cn(
            "text-grey-text-400 flex rounded-lg px-3 py-1.5 font-medium",
            {
              "tab-shadow text-grey-text-800 bg-white": table === "orders",
            },
          )}
        ...
        <button
          ...
          className={cn(
            "text-grey-text-400 flex rounded-lg px-3 py-1.5 font-medium",
            {
              "tab-shadow text-grey-text-800 bg-white": table === "activity",
            },
          )}
          onClick={() => selectTable("activity")}
        >
          User Activity
        </button>
      </div>
      ...
      {table === "activity" && <CustomerUserActivity customerId={customerId} />}
    </div>
  );
};
